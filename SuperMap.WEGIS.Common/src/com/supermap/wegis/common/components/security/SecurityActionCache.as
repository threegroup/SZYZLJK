package  com.supermap.wegis.common.components.security
{
	...
	public class SecurityActionCache
	{
		...
		public function getDelayLoadActions(comp:UIComponent):ArrayCollection {
			...
			for each(var da:SecurityAction in delayLoadActions) {
				if(da.componentId == comp.id) {
					if(hasParent(comp,da.parentComp as IVisualElementContainer))
						list.addItem(da);
				}						
			}
			return list;
		...
		private static function hasParent(comp:UIComponent, parent:IVisualElementContainer):Boolean {
			if(comp.parent != null && comp.parent is IVisualElementContainer) {
