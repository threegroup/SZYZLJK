package com.supermap.wegis.common.components.windowShade
{
	...
	import com.supermap.wegis.common.components.events.WindowShadeEvent;
	import com.supermap.wegis.common.skins.WindowShadeSkin;
	...
	public class WindowShade extends LayoutContainer {
		...
		[Embed (source="com/supermap/wegis/common/icons/assets.swf", symbol="right_arrow")]
		private static var DEFAULT_CLOSE_ICON:Class;
		
		[Embed (source="com/supermap/wegis/common/icons/assets.swf", symbol="down_arrow")]
		...
		private static function constructClass():Boolean {
			...
			var css:CSSStyleDeclaration = StyleManager.getStyleManager(null).getStyleDeclaration("WindowShade");
			...
			for(var styleProp:String in styleDefaults) {
				if(! StyleManager.getStyleManager(null).isValidStyleValue(css.getStyle(styleProp))) {
			...
			if(changed) {
				StyleManager.getStyleManager(null).setStyleDeclaration("WindowShade", css, true);
			}
		...
		protected function createOrReplaceHeaderButton():void {
			...
			if(_headerRenderer) {
				...
				if(headerClass == null)
				{
					headerClass = getDefinitionByName("spark.components.Button") as Class;
				}
				_headerButton = new headerClass();
				_headerButton.setStyle("skinClass", WindowShadeSkin);
				_headerButton.height = 20;
			}
		...
		protected function applyHeaderButtonStyles(button:Button):void {
			...
			if(_opened) {
				button.setStyle('iconCls', getStyle("openIcon"));
			}
			else {
				button.setStyle('iconCls', getStyle("closeIcon"));
			}
		...
		override protected function commitProperties():void {
			...
			if(_openedChanged) {
				...
				if(_opened) {
					_headerButton.setStyle('iconCls', getStyle("openIcon"));
				}
				else {
					_headerButton.setStyle('iconCls', getStyle("closeIcon"));
				}
