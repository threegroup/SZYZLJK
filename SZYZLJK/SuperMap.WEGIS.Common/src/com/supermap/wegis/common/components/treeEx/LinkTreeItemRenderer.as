...
{
	import com.supermap.wegis.common.components.treeEx.CheckTree;
	 ...
	 *  <br /><br />
	 */ 
	public class LinkTreeItemRenderer extends TreeItemRenderer
	{
		 ...
		 */
		static public var STATE_SCHRODINGER:int=2;
		static public var STATE_CHECKED:int=1;
		static public var STATE_UNCHECKED:int=0;
		private var myTree:LinkTree;
		/**能够展开的最后一级*/
		private var _leafLevel:int = -1;
		
		[Bindable]
		/**能够展开的最后一级*/
		public function get leafLevel():int
		{
			return _leafLevel;
		}
		/**能够展开的最后一级*/
		public function set leafLevel(value:int):void
		{
			_leafLevel = value;
		}
		
		public function LinkTreeItemRenderer()
		{
			super();
			mouseEnabled=true;
		}
		...
		{
			myTree = this.owner as LinkTree;
			myCheckBox=new CheckBox();
			if(myTree.hasCheckbox)
			{
				addChild(myCheckBox);
				myCheckBox.addEventListener(MouseEvent.CLICK, checkBoxToggleHandler);
				BindingUtils.bindProperty(myCheckBox,"enabled",myTree,"enabled");
			}else{
				if(myCheckBox != null && contains(myCheckBox))
				{
					removeChild(myCheckBox);
					myCheckBox.removeEventListener(MouseEvent.CLICK, checkBoxToggleHandler);
				}
			}
		...
		private function checkBoxToggleHandler(event:MouseEvent):void
		{
			if (data)
			{
				...
				myTree=myListData.owner as LinkTree;
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			if (super.data)
			{
				if (disclosureIcon)
				{
					...
					if("areaLevel" in data && leafLevel != -1 && data["areaLevel"] < leafLevel)
					{
						disclosureIcon.visible =  true;
					}
					if("children" in data && data["children"] is Array && data["children"].length > 0)
					{
						disclosureIcon.visible =  true;
					}
				}
