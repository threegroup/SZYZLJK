...
{
	import com.supermap.wegis.common.components.events.CheckTreeEvent;
	...
	public class CheckTreeItemRenderer extends TreeItemRenderer
	{
		...
		private var _isShowCheck:Boolean = true;
		
		/**是否添加checkbox*/
		public function get isShowCheck():Boolean
		{
			return _isShowCheck;
		}
		 ...
		 * @private
		 */
		public function set isShowCheck(value:Boolean):void
		{
			_isShowCheck = value;
		}
		...
		private function checkBoxToggleHandler(event:MouseEvent):void
		{
			...
			myTree.dispatchEvent(new CheckTreeEvent(CheckTreeEvent.CHECK_SELECT_CHANGE));
		}
		/**
		 * 设置本项的复选框状态
		 * @param checkBox 复选框
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			if (super.data)
			{
				if (disclosureIcon)
				{
					if(leafLevel != -1 && data["areaLevel"] < leafLevel)
					{
				...
				if (super.icon != null && isShowCheck)
				{
